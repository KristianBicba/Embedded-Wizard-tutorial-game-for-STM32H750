...
static const XRect _Const0008 = {{ 0, 0 }, { 1, 1 }};
  ...
  while ( view != 0 )
  {
    ...
    if ((( view->viewState & CoreViewStateVisible ) == CoreViewStateVisible ))
    {
      if ((( view->viewState & CoreViewStateEmbedded ) == CoreViewStateEmbedded ))
      {
        if ( outlineVisible )
        {
          XRect area = EwIntersectRect( CoreView__GetExtent( view ), clip );
      ...
      {
        XRect area = EwIntersectRect( CoreView__GetExtent( view ), aArea );
...
EW_DEFINE_CLASS( CoreGroup, CoreRectView, first, first, _.VMT, _.VMT, _.VMT, _.VMT, 
                 "Core::Group" )
  CoreView_GetRoot,
  CoreGroup_Draw,
  CoreView_HandleEvent,
  ...
  aFallbackTarget, CoreView aStartView, XSet aRetargetReason )
{
  ...
  hit = CoreView__CursorHitTest( _this, EwMoveRectPos( _Const0007, _this->cursorLastPos[ 
  ...
  aEvent )
{
  ...
  if ( release && ( _this->state < 16777216 ))
  {
    ...
    if ( root != 0 )
    {
      ...
      hit = CoreView__CursorHitTest( root, EwMoveRectPos( _Const0008, event1->GlobalCurrentPos ), 
